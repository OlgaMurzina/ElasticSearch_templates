...
Метод удаляет документ из индекса	Ничего
delete_by_query
(index, 
...
Метод удаляет документы, соответствующие заданному запросу	Ничего
exists
(index, 
...
Метод возвращает информацию о том, существует ли документ в индексе	True/False
exists_source
(index, 
...
Метод возвращает информацию о том, существует ли источник документа в индексе	True/False
field_caps
(body=None,
...
Метод возвращает информацию о “типах” одних и тех же полей в нескольких индексах	json
get
(index, 
...
Метод возвращает документ по  его ID	json
get_source
(index, 
...
Метод возвращает источник документа 	json
index
(index, 
...
Метод создает или обновляет документ в индексе	Ничего
info
(params=None,

headers=None)

-	Метод возвращает основную информацию о кластере	json
mget
(body, 
...
Метод позволяет получить несколько документов одним запросом	json
msearch
(body,
...
Метод позволяет выполнять несколько поисковых операций в одном запросе	json
open_point_in_time
(index=None,
...
Метод открывает момент времени, который можно использовать в последующих поисках	???
rank_eval
(body, 
...
Метод позволяет оценить качество ранжированных результатов поиска по набору типичных поисковых запросов	Возвращает типичные метрики поиска информации, такие как средний взаимный ранг, точность или дисконтированный кумулятивный выигрыш - json
reindex
(body, 
...
Метод позволяет копировать документы из одного индекса в другой, дополнительно фильтруя исходные документы по запросу, изменяя настройки целевого индекса или извлекая документы из удаленного кластера	Ничего
scroll
(body=None, 
...
Метод позволяет делать скролл поиска по scroll_id при больших кол-вах документов в индексе	json
search
(body=None, 
...
Метод возвращает результаты, соответствующие запросу	json
termvectors
(index, 
...
Метод возвращает информацию и статистику о терминах в полях конкретного документа	json
update
(index, 
...
Метод выполняет обновление для каждого документа в индексе без изменения источника, например, для получения изменения сопоставления
